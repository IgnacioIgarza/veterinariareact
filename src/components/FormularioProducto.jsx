...
function FormularioProducto() {
  ...
  useEffect(() => {
    if (modo === "editar" && productoRecibido) {
      setProducto({
        ...
        masinfo: productoRecibido.masinfo || ''
      });
    }
  ...
  const manejarEnvio = async (e) => {
    ...
    try {
      ...
      toast(`Hubo un problema al ${modo === "editar" ? 'actualizar' : 'agregar'} el producto`);
    ...
    <form onSubmit={manejarEnvio} style={{ maxWidth: '600px', margin: '0 auto', padding: '20px' }}>
      ...
      <div style={{ marginBottom: '15px' }}>
        ...
        <input
          ...
          onChange={manejarCambio}
          disabled={cargando}
          placeholder="Ej: Juguete, Salud, Higiene, etc."
          style={{
            width: '100%',
            padding: '8px',
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <textarea
          name="masinfo"
          value={producto.masinfo}
          onChange={manejarCambio}
          rows="5"
          disabled={cargando}
          maxLength="300"
          placeholder="Mínimo 10 caracteres, máximo 300 caracteres"
          style={{
            width: '100%',
            padding: '8px',
            border: `1px solid ${errores.masinfo ? 'red' : '#ccc'}`,
            borderRadius: '4px',
            resize: 'vertical'
          }}
        />
        <div style={{
          fontSize: '12px',
          color: producto.masinfo.length > 300 ? 'red' : '#666',
          marginTop: '5px'
        }}>
          {producto.masinfo.length}/300 caracteres
        </div>
        {errores.masinfo && (
          <p style={{ color: 'red', margin: '5px 0', fontSize: '14px' }}>{errores.masinfo}</p>
        )}
      </div>
