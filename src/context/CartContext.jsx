...
export function CartProvider({ children }) {
  ...
  const [cargaCompleta, setCargaCompleta] = useState(false); //Flag para carga completa
  ...
  useEffect(() => {
    // Guardar el carrito en localStorage cuando cambie, pero sólo después de la carga inicial
    if (cargaCompleta) {
  ...
  const eliminarDelCarrito = (productoId) => {
   ...
   const quitarCantidad = (idProducto) => {
    const carritoActualizado = carrito.map(producto => {
      ...
      toast.success(`1 ${producto.nombre} eliminado.`);
      return producto;
    ...
    const nuevoCarrito = carrito.map(producto => {
      ...
      toast.success(`1 ${producto.nombre} agregado.`);
      return producto;
    });
